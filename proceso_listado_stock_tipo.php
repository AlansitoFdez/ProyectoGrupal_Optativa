...
$id_tipo_seleccionado = $_GET['lstTipo'];
...
$sql = "
    ...
    WHERE s.stock_type_id_FK = " . intval($id_tipo_seleccionado) . "
...
if (!$resultado) {
    ...
    if (mysqli_num_rows($resultado) > 0) {
        $mensaje = "<h2 class='text-center'>Listado de Stocks por Tipo</h2>";
        ...
        while ($fila = mysqli_fetch_assoc($resultado)) {
            ...
            $mensaje .= "<button type='submit' name='btnEditar' class='btn btn-warning'><i class='bi bi-pencil'></i></button></form>";
            $mensaje .= "<form action='proceso_borrar_stock.php' method='post' style='display:inline-block;'>";
            $mensaje .= "<input type='hidden' name='id_stock' value='$id_stock' />";
            $mensaje .= "<button type='submit' name='btnBorrar' class='btn btn-danger'><i class='bi bi-trash'></i></button></form></td>";

            $mensaje .= "</tr>";
        }
        $mensaje .= "</tbody></table>";
    } else {
        $mensaje = "<h2 class='text-center mt-5'>No hay stocks para este tipo seleccionado</h2>";
    }
}
